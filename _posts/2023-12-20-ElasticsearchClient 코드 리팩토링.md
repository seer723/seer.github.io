...
if 구문으로 로직이 나뉘어있어서 소스를 볼 때 if 절과 else 절을 두 번 확인해야하는 불편함이 있었는데요.  
...
만약 검색 함수가 100개가 되었을 때 구현 코드가 어떨지 상상을 해봤어요.  
반복된 코드가 많이 나타날 것이라고 예상할 수 있죠.  
그래서 자주 쓰이는 코드를 따로 함수로 분리해서 코드 재사용성을 높이는 작업을 했어요.  
나중에 새로운 검색 함수가 추가될 때 term 이나 matchQuery 함수를 사용하면 코드 중복을 피할 수 있어요.